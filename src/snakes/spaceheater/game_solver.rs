...
use crate::{
    log,
    logic::{scoring::ApproximateScore, search, BoardLike, Game, Snake, Tile},
        ...
        if certain_death(
            ...
            for dir in ALL_DIRECTIONS {
                if !certain_death(
                    ...
                    top_dir = dir;
                    top_score = self.scores.get(&vec![dir]);
                }
            }
        }
    ...
    for snake in &game.others {
        let mut viable_directions: Vec<Direction> = ALL_DIRECTIONS
            ...
            .filter(|&dir| {
                ...
                let mut survives = !certain_death(game, &snake, &pos, snake.health);
                if snake.length > 1 {
